...
def download_overpass_query(query, filepath, timeout=1500):
    ...
    with open(filepath, 'w+') as file:
        file.write(get_response_from_overpass_server(server, query, timeout))

def get_response_from_overpass_server(api_url, query, timeout):
    #print("sleeping before download")
    #sleep(20)
    while True:
        try:
            response = single_query_run(api_url, query, timeout)
...
def single_query_run(api_url, query, timeout):
